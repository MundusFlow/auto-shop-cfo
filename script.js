...
const UPGRADE_HTML =
  ...
  '<a href="#cta">Get The Auto Shop CFO &rarr;</a>';
...
// ---------------------------------------------------------------------------
(() => {
  const partsInput = document.getElementById("q_parts");
  const markupInput = document.getElementById("q_partsMarkup");
  const hoursInput = document.getElementById("q_laborHours");
  const rateInput = document.getElementById("q_laborRate");
  const suppliesInput = document.getElementById("q_supplies");
  const marginInput = document.getElementById("q_profitMargin");
  const btn = document.getElementById("quoteBtn");
  const resultEl = document.getElementById("quoteResult");

  wireUpForm([partsInput, hoursInput, rateInput, suppliesInput], btn);
  wireClearButton("q_clearBtn", [partsInput, markupInput, hoursInput, rateInput, suppliesInput, marginInput], btn, resultEl);

  function calculate() {
    const parts = num(partsInput);
    ...
    const partsWithMarkup = parts * (1 + markupPct / 100);
    const laborCost = hours * rate;
    const subtotal = partsWithMarkup + laborCost + supplies;
    const profitBuffer = subtotal * (marginPct / 100);
    const quoteTotal = subtotal + profitBuffer;

    const interpretation = `Charging ${formatMoney(quoteTotal)} covers your ${formatMoney(subtotal)} in parts, labor, and supplies, and still bags you ${formatMoney(profitBuffer)} in profit &mdash; right at your ${formatPercent(marginPct)} target margin.`;
    const benchmark = "Typical gross margin for independent auto repair shops: roughly 45-60% (general estimate).";
    ...
    resultEl.innerHTML = `
      <div class="result-box">
        ...
        <div class="result-breakdown">
          ...
          <div class="breakdown-row"><span>Parts (+${formatPercent(markupPct)} markup)</span><span>${formatMoney(partsWithMarkup)}</span></div>
          <div class="breakdown-row"><span>Labor (${hours} hrs &times; ${formatMoney(rate)})</span><span>${formatMoney(laborCost)}</span></div>
          <div class="breakdown-row"><span>Shop supplies / fees</span><span>${formatMoney(supplies)}</span></div>
...
(() => {
  ...
  const partsInput = document.getElementById("jp_parts");
  ...
  wireUpForm([invoicedInput, partsInput, laborInput, otherInput], btn);
  wireClearButton("jp_clearBtn", [invoicedInput, partsInput, laborInput, otherInput], btn, resultEl);
  ...
  function calculate() {
    ...
    const totalCost = parts + labor + other;
    ...
    resultEl.innerHTML = `
      <div class="result-box">
        ...
        <div class="result-breakdown">
          ...
          <div class="breakdown-row"><span>&minus; Parts</span><span>${formatMoney(parts)}</span></div>
          <div class="breakdown-row"><span>&minus; Technician labor</span><span>${formatMoney(labor)}</span></div>
          <div class="breakdown-row"><span>&minus; Other costs</span><span>${formatMoney(other)}</span></div>
        ...
        ${followUpBlock("#labor-rate", "Labor Rate Calculator")}
      </div>
    `;
  }
...
(() => {
  ...
  const rentInput = document.getElementById("hr_rent");
  const insuranceInput = document.getElementById("hr_insurance");
  const equipmentInput = document.getElementById("hr_equipment");
  ...
  wireClearButton(
    "hr_clearBtn",
    [takeHomeInput, rentInput, insuranceInput, equipmentInput, softwareInput, otherOverheadInput, hoursPerWeekInput, weeksPerYearInput],
    btn,
    resultEl
  );
  ...
  function calculate() {
    ...
    const monthlyOverhead =
      num(rentInput) + num(insuranceInput) + num(equipmentInput) + num(softwareInput) + num(otherOverheadInput);
    ...
    const benchmark = "Independent shop labor rates commonly range $90-$180/hr depending on region (rough estimate).";
...
(() => {
  ...
  const partsInput = document.getElementById("tax_parts");
  ...
  wireUpForm([incomeInput, partsInput, expensesInput, bracketSelect], btn);
  wireClearButton("tax_clearBtn", [incomeInput, partsInput, expensesInput, bracketSelect], btn, resultEl);
  ...
  function calculate() {
    ...
    const netProfit = Math.max(income - parts - expenses, 0);
    ...
    const interpretation = `Setting aside ${formatMoney(totalReserve)} (about ${(reserveRate * 100).toFixed(1)}% of your ${formatMoney(netProfit)} profit) leaves you ${formatMoney(spendable)} that's actually yours to spend or reinvest.`;
    ...
    resultEl.innerHTML = `
      <div class="result-box">
        ...
        <div class="result-breakdown">
          ...
          <div class="breakdown-row"><span>&minus; Parts / COGS</span><span>${formatMoney(parts)}</span></div>
  ...
  function calculate() {
    ...
    resultEl.innerHTML = `
      <div class="result-box">
        ...
        ${followUpBlock("#quote-builder", "Quote Builder")}
      </div>
    `;
  }
